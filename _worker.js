...
// ─────────────────────────────────────────────────────────────
// 8. Telnyx TeXML Interactive Inbound IVR & SIP Bridging Engine
// ─────────────────────────────────────────────────────────────

/**
 * Handles Inbound Phone Calls on +1-866-546-9311 (1-866-LIMYE-11)
 * Features:
 *  - Option 2 DTMF IVR Anti-Spam Gate (drops silent robocallers & scrapers before AI minutes accrue)
 *  - Categorized Department Routing:
 *      1 -> Caseworkers / Agency Partners
 *      2 -> Direct Housing, Nutrition & Community Assistance
 *      3 -> Landlords, Donors & Community Partners
 *      4 -> Executive Management & General Inquiries
 *  - Bridges to Fanvil V67 Desk Phone (Line 4: sip:limyefoundation@sip.telnyx.com) with 15-second ring
 *  - Unanswered Fallback to Harry AI Voice Assistant (assistant-daa8fc21-90e1-44f7-8702-cad5e6587128)
 */
async function handleTelnyxInboundCall(request, env, ctx) {
    try {
    ...
    <Hangup />
        ...
        if (step === 'option_selected') {
            ...
            if (digits === '1') {
                ...
                // Invalid digit pressed or no digit
                const invalidXml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say voice="Polly.Stephen-Neural">Invalid selection. Goodbye.</Say>
    <Hangup />
</Response>`;
                return new Response(invalidXml, { headers: { 'Content-Type': 'application/xml; charset=utf-8' } });
            }

            const callerTag = `${tagPrefix} ${cleanFrom}`.substring(0, 30);
            const sipUri = 'sip:limyefoundation@sip.telnyx.com';

            // Ring Fanvil desk phone for 15 seconds (approx 3 rings). If no answer, fallback to Harry AI!
            const xml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Dial timeout="15" callerId="${callerTag}" action="${baseUrl}/api/texml/inbound?step=harry_fallback&amp;category=${encodeURIComponent(catName)}&amp;from=${encodeURIComponent(from)}&amp;to=${encodeURIComponent(to)}">
        ...
        // --- STEP 3: HARRY AI ASSISTANT FALLBACK (ON NO-ANSWER / BUSY) ---
...
<Response>
    <Redirect method="POST">${baseUrl}/api/texml/inbound?step=menu&amp;from=${encodeURIComponent(from)}&amp;to=${encodeURIComponent(to)}</Redirect>
        ...
        return new Response(defaultXml, { headers: { 'Content-Type': 'application/xml; charset=utf-8' } });
    } catch (err) {
...
// ─────────────────────────────────────────────────────────────
// 9. Cloudflare Worker Entry Point
// ─────────────────────────────────────────────────────────────
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method === 'OPTIONS') {
            const apiRoutes = ['/api/contact', '/api/sms', '/api/telnyx/webhook', '/api/telnyx/voice-webhook', '/api/vapi/webhook', '/api/texml/inbound', '/api/telnyx/inbound-voice', '/api/submit-housing-form', '/api/submit-contact-form', '/api/subscribe-newsletter'];
        ...
        // 3. API Endpoints
        if ((request.method === 'POST' || request.method === 'GET') && (url.pathname === '/api/texml/inbound' || url.pathname === '/api/telnyx/inbound-voice')) {
            return handleTelnyxInboundCall(request, env, ctx);
        }
